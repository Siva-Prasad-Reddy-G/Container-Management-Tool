...
run()
{
    ...
    chmod 755 $CONTAINERDIR/$NAME/rootfs /bin/bash
  
    unshare  --uts --pid --n --i --mount --kill-child  chroot $CONTAINERDIR/$NAME/rootfs  /bin/bash -c " mount -t sysfs sysfs /sys && mount -t proc proc /proc && $INIT_CMD_ARGS"  
...
exec()
{
    ...
    nsenter --target $CONTAINER_INIT_PID --pid --mount --uts --ipc --net -w -r  $EXEC_CMD_ARGS
