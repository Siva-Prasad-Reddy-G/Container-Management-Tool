...
#define CHILD_STACK_SIZE 0x800001
...
int main() {
   ...
   child_pid=clone(child_function, (char *) child_stack+ CHILD_STACK_SIZE, CLONE_NEWUTS | CLONE_NEWPID |SIGCHLD, pipefd);
    ...
    */
    ...
    // ------------------ WRITE CODE HERE ------------------
     ...
     int uts_fd, pid_fd;
     char uts_path[256], pid_path[256];

     
     sprintf(uts_path, "/proc/%d/ns/uts", child_pid);
     sprintf(pid_path, "/proc/%d/ns/pid", child_pid);

     uts_fd=open(uts_path, O_RDONLY);
     pid_fd=open(pid_path, O_RDONLY);
     if(uts_fd==-1 || pid_fd==-1)
	     perror("Wrong fd");
     if(setns(pid_fd,CLONE_NEWPID)==-1) perror("PID error");
    ...
    if (fork() == 0) {
        ...
        */
        ...
        // ------------------ WRITE CODE HERE ------------------
	    if(setns(uts_fd,CLONE_NEWUTS)==-1) perror("UTS error");
